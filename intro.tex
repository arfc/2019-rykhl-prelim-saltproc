...
In contrast with conventional solid-fueled reactors, which are completely 
replace the fuel every 4-5 years\footnote{For the most common 18-month cycle, 
...
lifetime. Therefore, some \glspl{FP}, not captured by fuel  reprocessing 
system, and transuranic elements\footnote{The chemical elements with atomic 
numbers greater than uranium (92).} are accumulating in the fuel salt. 
continuous changing of fuel salt composition have a significant influence on 
...
coefficient at start-up and at equilibrium state. The temperature coefficient 
...
\delta T &= 1200K-900K \nonumber
\end{align}
That work showed negative and decreasing with time fuel salt temperature 
coefficient ($-1.5$ and $-1.0pcm/K$ at start-up and at equilibrium state, 
respectively), but positive total temperature coefficient which did not 
change during reactor operation ($\approx+0.8pcm/K$).

\subsection{Transatomic Power Reactor design overview}
